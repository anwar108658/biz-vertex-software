...
const Navbar = () => {
    ...
    const checkMenuHandler = (val:string) => {
        dispatch(isMenuHandle(val))
  ...
  return (
    <div className='grid grid-cols-6 lg:grid-cols-12 gap-2 p-[1px] py-[3px] px-2 bg-[#292D2D] text-white'>
        ...
        <div className="col-span-5 order-1 sm:order-0 grid grid-cols-12 gap-1 py-1">
            ...
            <div className="col-span-11 bg-[#404648]"><input onChange={(e) => dispatch(onChangeVal(e.target.value),console.log("input",e.target.value))} value={store.data} className="w-full h-full focus:outline-none p-2" type="text" /></div>
        </div>
        ...
        <ul className="col-span-2 flex gap-2 items-center">
            ...
            <li onClick={() => checkMenuHandler("func")} className="flex-1 flex items-center justify-center cursor-pointer border-b-2 border-b-[#292D2D] hover:border-b-2 hover:border-b-[#009DDF] hover:bg-[#3E4242] h-full">Function</li>
        </ul>
        <div className="col-span-1 flex justify-between gap-2 items-center">
            <button onClick={() => checkMenuHandler("search")}>
             <Search/>
            </button>
            <button>
             <Person/>
